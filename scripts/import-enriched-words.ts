...
async function main() {
  ...
  for (let index = 0; index < filteredItems.length; index += CHUNK_SIZE) {
    ...
    await importChunk(supabase, chunk, summary, options.syncPrimaryExample, options.dryRun)
  }
...
async function importChunk(
  ...
  const profileRows = chunk.map((item) => ({
    ...
    semantic_feel: item.profile.semanticFeel,
    usage_note: item.profile.usageNote,
    usage_register: item.profile.usageRegister,
    scene_tags: item.profile.sceneTags,
    collocations: item.profile.collocations,
    contrast_words: item.profile.contrastWords,
  ...
  const primaryExampleRows = syncPrimaryExample
    ? chunk
        .map((item) => ({
          wordId: item.wordId,
